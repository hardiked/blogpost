		...
		<div class="col-md-6 col-md-offset-3">
			...
			<form action="{{ route('post.create') }}" method="post">
				<div class="form-group">
					<textarea class="form-control" name="body" id="new-post" rows="5" placeholder="YourPost"></textarea>
				</div>
			...
			@foreach($posts as $post)
				<article class="post" data-postid="{{ $post->id }}">
					...
					<div class="interaction">
						...
						<a href="#" class="like">{{ Auth::user()->likes()->where('post_id',$post->id)->first()
						? Auth::user()->likes()->where('post_id',$post->id)->first()->like==0?'You don\'t like this post':'Dislike' : 'Disike'}}</a> 
						@if(Auth::user()==$post->user)
						|
						<a data-toggle="modal" data-target="#editModal" href="#" class="edit">Edit</a> |
						<a href="{{ route('post.delete',['post_id'=>$post->id]) }}">Deletes</a>
						@endif
